    ...
    def chat_thread():
        ...
        for chunk in stream:
            if chunk and 'message' in chunk and chunk['message'].content:
                ...
                # Separar el pensamiento del resultado final
                if "think" in chunk_content:
                    thought += chunk_content
                else:
                    content += chunk_content
                    # Detectar el idioma del contenido
                    if detect(chunk_content) == 'es':
                        spanish_content += chunk_content

        response = content.strip()

        # Imprimir el pensamiento en la pestaña de pensamiento
        thought_area.insert(ctk.END, f"Pensamiento: {thought}\n", "thought_font")
        thought_area.see(ctk.END)
        ...
        # Decir en voz alta solo el contenido en español
        if spanish_content:
            engine.say(spanish_content)
            engine.runAndWait()
...
# Crear una pestaña para el pensamiento de la máquina
thought_frame = ctk.CTkFrame(root)
thought_frame.pack(padx=10, pady=10, fill="both", expand=True)

# Crear un área de texto para el pensamiento de la máquina
thought_area = Text(thought_frame, wrap="word", width=60, height=10, font=("Helvetica", 12))
thought_area.pack(padx=10, pady=10)

# Configurar la fuente para la respuesta de la IA y el pensamiento
...
thought_area.tag_configure("thought_font", font=("Courier", 12, "italic"))
